...
def length_words(words):
    Lenghts_of_word_list = []
    for word in words:
        Lenghts_of_word_list.append(len(word))

    return Lenghts_of_word_list
...
def extract_data(data):
    result=[]
    #iteration over each city:
    for city_name, city_weather_info in data.items():
        #iteration over the two records per city
        for record in city_weather_info['weather_list']:
            result.append({
                'name' : city_name,
                'country' : city_weather_info['metadata']['country'],
                'date' :   record['dt_txt'],
                'temp' : record['main']['temp'] -273.15
            })
